#!/usr/bin/env python3
"""深知可信PPT 溯源核验报告渲染器：把检索素材 JSON + 答案文件渲染为单文件可核验 HTML。
...
- 双形态 JSON：素材数组认顶层 {"检索文章": [...]} 或原始接口 content.data.检索文章
- 未引用判定双通道：JSON 标记（recalled_materials）∪ 正文角标未引用
- 去重键含角标：同一文章被赋不同角标时各留一卡；双向硬校验（exit 2）
...
# verify_snapshots 在展示前完成"补 /A/ 修订 + 纯本地格式校验 + 非法弃用"。
...
def normalize_kind(raw: str) -> Tuple[str, str]:
    """把来源类型文本归一化为（类型键, 中文标签）；css 类名查 KIND_CATALOG。"""
    text = str(raw or "")
...
# ============================================================
# 素材提取（数据层：材料级编号优先 + 位置兜底 + 双形态 JSON）
# ============================================================
...
def clean_paragraph_noise(text: str) -> str:
    ...
    URL 前是段落标题（已由标题链单独展示），URL 本身是附件地址（卡片有"查看全文"入口）。
...
def source_from_article(item: Dict[str, Any], index: int, segment: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    segment = segment or {}
    # 材料级编号（角标/编号/materialIndex 等）优先；段落 id 是文章内段落序号（各篇独立
    # 从 1 起，多篇同为 "1" 会互相冲突导致角标错乱），彻底不用作材料编号。
...
def collect_raw_articles(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    """链接检测/快照校验的作用对象：全部原始材料 dict（含未引用召回），就地修改生效。"""
    raws: List[Dict[str, Any]] = []
    raws.extend(extract_reference_materials(payload))
    raws.extend(extract_articles_from_search(payload))
    raws.extend(extract_articles_from_deep(payload))
    recalled = payload.get("recalled_materials") or payload.get("召回材料")
    if isinstance(recalled, list):
        raws.extend(x for x in recalled if isinstance(x, dict))
    return raws
...
def extract_sources(payload: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    # 未引用召回材料（接口召回但正文未采用，agent 筛选后全量放入 recalled_materials）：
    # 进面板「未引用」分组，不占正文角标编号，不参与核验统计。
    recalled = payload.get("recalled_materials") or payload.get("召回材料")
    if isinstance(recalled, list):
...
def compute_verification(answer: str, sources: List[Dict[str, str]], payload: Dict[str, Any],
    ...
    # 不应拖累"正文依据可交付"的报告级结论。
    # PPT 提醒制：缺原文链接（接口对部分治理库文章不返回源网址，属数据源覆盖问题）与
    # 缺摘录均降级为提醒，不参与核验判定——报告级只拦真实错误。
    ...
    # 核验判定（PPT 提醒制）：只有真实错误才判不通过（无素材 / 正文无角标 / 角标未绑定）；
    # 数据缺口（缺链/缺摘录）只提醒不影响判定。
    trace_ok = bool(cited_sources)
...
def parse_answer_blocks(answer: str, valid_ids: set, chip_map: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], set]:
    """把正文 Markdown 解析为结构化块；同时收集全部角标。

    角标渲染为原型式 jb 胶囊（编号圆徽章 + 材料标题），点击原地展开溯源卡；
    表格单元格内的角标同样就地变胶囊（证据在含角标的单元格内，不挂表外）。
    """
    ...
    def make_block_repl(block_text: str):
        """按块构造角标替换器：同块同材料只保留最后一次出现的角标（前处整体删除），
        最后一处挂 jb 胶囊——一块中同一材料只出现一次，杜绝重复引用刷屏。"""
...
def group_sections(blocks: List[Dict[str, Any]]) -> Tuple[Optional[str], List[Dict[str, Any]]]:
    """按标题层级切分章节。返回（报告标题, 章节列表）。

    规则：最浅层级标题若唯一且居文首，视为报告标题提出；其下一级为章节边界。
    无标题的短正文整篇一节。
    """
    if not blocks:
...
def build_chip_data(sources: List[Dict[str, str]]) -> Dict[str, Dict[str, Any]]:
    """行内引文胶囊数据：id → {title, site, url, excerpt, segments}。
...
def render_verify_panel(v: Dict[str, Any], stage: str = "final") -> str:
    """核验报告单：报告级核验结论 + 五项指标（PPT 双版 + 提醒制口径）。"""
    ...
    cov_html = (f'<div class="vi"><span class="s {cov_state}">{"✓" if cov_state == "ok" else "—"} 材料构成</span>'
                f'<span class="d">{esc(" · ".join(parts)) if parts else "材料没标类型（合并 JSON 可加「类型」字段）"}</span></div>')
    ...
    if sc["status"] == "pass":
        ...
        default_names = "、".join(label for _, label in SELF_CHECK_ITEMS)
...
def render_section_cards(sections: List[Dict[str, Any]], sources: List[Dict[str, str]]) -> str:
    """正文渲染（原型复刻）：chapter（h2）+ lead/para 段落文档流 + 句后 jb 引文胶囊。

    同一块内同一材料只保留最后一处角标与胶囊（make_block_repl 去重）；
    提纲页面规划表等 markdown 表格内角标同样就地变胶囊（证据在含角标的单元格内）。
    """
    parts: List[str] = []
    first_para = True
    source_ids = {s["id"] for s in sources}
...
def render_source_card(source: Dict[str, str], for_print: bool = False) -> str:
    if not source.get("used", True):
        ...
        if not source.get("has_link"):
            vk += '<span class="sc-vk warn">◐ 无原文链接</span>'
...
def apply_snapshots(articles: List[Dict[str, Any]]) -> Tuple[int, int]:
    """快照兜底预处理：路径补全（/A/ 层级）+ 纯本地格式校验，非法路径弃用。
    ...
    fixed = dropped = 0
    for a in articles:
        raw = first_str(a.get("快照链接"), a.get("snapshot"), a.get("screenShotPath"), a.get("screenshot_path"))
        if not raw:
            continue
        normalized = normalize_snapshot_url(raw)
        if normalized != raw:
            fixed += 1
        if normalized.startswith("https://attach.dknowc.cn/snapshot/A/"):
            a["快照链接"] = normalized
        else:
            a["快照链接"] = ""
            dropped += 1
...
@keyframes cardFlash{0%,100%{box-shadow:none}30%{box-shadow:0 0 0 3px rgba(139,92,246,.45)}}
...
(function () {
  ...
  function sentenceOf(el) {
    var para = el && el.closest ? el.closest(".para,.lead") : null;
    return para ? para.innerText.trim() : "";
  }
  ...
  /* 行内引文 jb（原型行为）：点小胶囊 toggle 展开卡；展开卡内引文条目点击进专库。
     未绑定角标红色警示，绝不按位置猜测。 */
  var JB_TABLES = {};
  try {
  ...
  document.querySelectorAll(".jb-quote").forEach(function (q) {
    function go() {
      ...
      if (isMobile()) {
        /* 移动端不跳视图（跳转会打断阅读），就地展开/收起当前 jb */
        var card = id && byId[id];
        if (card) { openSheet(card, sentenceOf(q)); return; }
      }
...
def render_html(payload: Dict[str, Any], title: str, answer_override: str = "", question_override: str = "",
    ...
    for cid, jb in chip_map.items():
        for key, tbl_html in (jb.pop("_tables", {}) or {}).items():
            jb_tables[key] = tbl_html
...
<header class="topbar">
  <div class="tb-left">
    <span class="tb-stamp">{esc(REPORT_NAME)}</span>
...
def main() -> None:
    parser = argparse.ArgumentParser(description="生成深知可信PPT 溯源核验报告 HTML（提纲版/成稿版）")
    ...
    parser.add_argument("--title", default="溯源核验报告", help="演示标题；正文首行有标题时以正文为准")
    ...
    if SNAPSHOT_ENABLED and raw_articles:
        ...
        if total_snap or fixed or dropped:
            print(f"快照校验：{total_snap} 条路径合法放行（补全 /A/ {fixed} 条）/ 弃用 {dropped} 条非法路径（不做网络检测）")

    # 生成前硬校验：有素材而正文无角标时拒绝生成，杜绝产出无核验对应的报告
    pre_answer = normalize_citations(answer_override) if answer_override.strip() else extract_answer(unwrapped)
    pre_sources = extract_sources(unwrapped)
