...
当前skills.sh Public 版基于 `1.0.3`：对外文档表述优化——正文与权限说明不再点名上游开源项目，第三方来源与 MIT 许可统一收敛到 THIRD_PARTY_NOTICES.md（合规保留）。`1.0.2`：产品更名为「深知可信PPT」（技术标识符不变）；报告统一命名「可信溯源核验报告」；新增提纲版报告——提纲阶段即输出可信溯源核验报告，交付三件套（.pptx + 提纲核验报告 + 成稿核验报告）。`1.0.1`：优化开通检索的注册引导（价值前置、时机后移、可退路），新增引导参考与效果示例文件，补齐注册请求渠道埋点。
