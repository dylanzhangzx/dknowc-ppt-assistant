...
- **内容可信**：所有事实素材来自深知可信搜索的权威文件库；每个数据、每条政策有来源。双版溯源核验报告：提纲确认前（事前核验）与交付时（事后溯源）各一份——首屏核验报告单（依据溯源/引用绑定/时效检查/类型覆盖/自检五项真实计算指标）、正文分节卡片、素材四分类色卡、打印归档模式，随 .pptx 三件套交付。
